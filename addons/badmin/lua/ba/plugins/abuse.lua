...
local cmd = ba.cmd.Create('Jail', function(pl, args)
	...
	if args.target:GetBVar('JailProps') then
		...
		ba.notify_staff(term.Get('AbuseAdminUnjailedPlayer'), pl, args.target)
		return
	end
	...
	ba.notify_staff(term.Get('AbuseAdminJailedPlayer'), pl, args.target)
end)
cmd:AddParam('player_entity', 'target')
cmd:SetFlag('S')
cmd:SetHelp('Садит в тюрьму игрока. Пример: /jail (ник) (срок) (причина)')
...
:SetFlag('O')
:SetHelp('Убивает игрока. Пример: /slay (ник)')
