...
function IGS.UI()
	...
	mf = uigs.Create("ui_frame", function(self)
	...
	-- Баланс
	uigs.Create("ui_button", function(self)
		...
		local add = uigs.Create("ui_button", mf)
		add:SetFont('ForkAwesome')
		add:SetPos(9 + 150 + 2, 0)
		add:SetSize(27, 29)
		add:SetText(utf8.char(0xf067))
	...
	function mf.sidebar:AddPage(sTitle)
		return uigs.Create("ui_panel", function(bg)
			bg.side = uigs.Create("ui_scrollpanel")
...
function IGS.AddTextBlock(scroll,sTitle,sText) -- используется в фрейме хелпа и чартов
	-- \/ вставленная панель
	return scroll:AddItem(uigs.Create("ui_panel", function(pnl)
		local y = 3

		-- Title
		if sTitle then
			for _,line in ipairs( string.Wrap("ui.20",sTitle,scroll:GetWide() - 5 - 5) ) do
		...
		for _,line in ipairs( string.Wrap("ui.18",sText,scroll:GetWide() - 5 - 5) ) do
...
function IGS.AddButton(pScroll,sName,fDoClick) -- используется в инвентаре и профиле для юза купонов
	-- \/ вставленная панель
	return pScroll:AddItem(uigs.Create("ui_panel", function(pan)
		pan.button = uigs.Create("ui_button", function(s)
