...
function PANEL:Init()
	self.head = uigs.Create("ui_panel", self)
	...
	self.scroll = uigs.Create("ui_scrollpanel", self)
...
function PANEL:SetIcon(sIco, bIsModel) -- url, Material or Model path
	...
	self.icon_bg = self.icon_bg or uigs.Create("ui_panel", function(bg)
...
function PANEL:SetSubNameButton(sName,func)
	...
	self.sub = self.sub or uigs.Create("ui_button", function(btn)
		btn:Dock(TOP)
		btn:SetTall(20)
		btn:SetFont("ui.20")
...
function PANEL:CreateActivity()
	self.activity = self.activity or uigs.Create("ui_panel", function(btn)
		btn:Dock(FILL)
	end, self.head)
...
function PANEL:AddPanel(sTitle,panel)
	...
	local background = uigs.Create("ui_panel", function(bg)
...
function PANEL:SetDescription(sDescription)
	...
	local pnl = uigs.Create("ui_panel")
...
function PANEL:SetInfo(tInf)
	...
	for k,v in pairs(tInf) do
		local line_bg = uigs.Create("ui_panel", pnl)
		line_bg:SetTall(15)
		line_bg:Dock(TOP)
...
vgui.Register("igs_iteminfo", PANEL, "ui_panel")
