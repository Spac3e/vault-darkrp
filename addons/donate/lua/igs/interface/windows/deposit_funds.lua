...
function IGS.WIN.Deposit(iRealSum)
	...
	m = uigs.Create("ui_frame", function(self)
		...
		uigs.Create("DLabel", function(real)
			...
			real:SetFont("ui.22")
			real:SetTextColor(ui.col.White)
		...
		self.purchase = uigs.Create("ui_button", function(p)
			local _,ry = self.real_m:GetPos()

			p:SetSize(400,40)
			p:SetDisabled(false) -- выделяет синим
		...
		--[[-------------------------------------------------------------------------
			Все подряд
		---------------------------------------------------------------------------]]
		self.log = uigs.Create("ui_scrollpanel", function(log)
			log:SetSize(250,200)
			...
			function log:AddRecord(text,pay)
				...
				return log:AddItem( uigs.Create("ui_panel", function(bg)
					...
					for i,line in ipairs(string.Wrap("ui.18",text,log:GetWide() - 0 - 0)) do
						uigs.Create("DLabel", function(l)
							...
							l:SetFont("ui.18")
							l:SizeToContents()
							l:SetTextColor(i == 1 and ui.col.Grey or col)
		...
		local log_t = uigs.Create("DLabel", function(log_title)
			...
			log_title:SetTextColor(ui.col.White)
		...
		uigs.Create("ui_panel", function(line)
		...
		uigs.Create("ui_button", function(btn)
