...
function BM2OpenTerminal(entity)
	...
	frame.Paint = function(s , w , h)
		...
		draw.RoundedBoxEx(8, 0, 0, w, h, Color(255, 155, 55, 255), true, true, false, false)
		//Draw background
	...
	consoleDisplay.history = consoleDisplay.history.."Найденная память (256mb)\n"
	consoleDisplay.history = consoleDisplay.history.."Найдена OS\n"
	consoleDisplay.history = consoleDisplay.history.."Загрузка завершена OS\n"
	consoleDisplay.history = consoleDisplay.history.."Запуск BitOS 1.0\n"
	consoleDisplay.history = consoleDisplay.history.."---------------------------------------\n"
	consoleDisplay.history = consoleDisplay.history.."Чтобы узнать команды, введите 'help'!\n"
	consoleDisplay.history = consoleDisplay.history.."\n\n"
...
local consoleColors = {
	default = Color(255, 155, 55),
	blue = Color(0,0,255),
	red = Color(255,0,0),
	green = Color(0,255,0),
	orange = Color(255,100,70),
	gray = Color(55,55,55),
	pink = Color(230,55,230)
}
...
local BM2DefaultIntructions = {
	...
	color = { //Changes console colors
		command = "COLOR",
		description = "Изменяет цвет консоли.",
		action = function(arg1, arg2, instructionTable, ent, consoleDisplay)
			if arg1 == nil then
				consoleDisplay.history = consoleDisplay.history.."\n-------------------COLOR-------------------\n"
				consoleDisplay.history = consoleDisplay.history.."Чтобы изменить цвет консоли, введите 'color name'\n"
				...
				if #string.Explode(",", arg1) == 3 then
					...
					consoleDisplay.history = consoleDisplay.history.."Цвет был изменен.\n"
				else
					if consoleColors[string.lower(arg1)] ~= nil then
						backgroundColor = consoleColors[string.lower(arg1)]
						consoleDisplay.history = consoleDisplay.history.."Цвет был изменен.\n"
					else
						consoleDisplay.history = consoleDisplay.history.."Данный цвет '"..arg1.."' недоступен.\n"
					end
				end
			end
		end
