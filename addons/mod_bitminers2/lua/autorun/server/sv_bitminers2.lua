		...
		if e:GetClass() == "bm2_bitminer_rack" then
			if index > 0 and index < 9 then
				...
				if e.rack[index] ~= nil then
					...
					net.Start("BM2.Client.TerminalPrint")
					net.WriteString("Сервер был извлечен.")
					net.Send(ply)
				else
					net.Start("BM2.Client.TerminalPrint")
				...
				net.Start("BM2.Client.TerminalPrint")
				net.WriteString("Указанный вами номер сервера не находится в диапазоне (1-8)")
				net.Send(ply)
			end
		end
	end
			...
			if type then
				...
				if e.upgrades.CORES.cost[e.upgradeTracker.cores + 1] ~= nil then
					if ply:canAfford(e.upgrades.CORES.cost[e.upgradeTracker.cores + 1]) then
						...
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Вы улучшили (CORE), введите info, чтобы увидеть новые спецификации!")
						net.Send(ply)
					else
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Вы не можете позволить себе это!")
						net.Send(ply)
					end
				end
			else
				...
				if e.upgrades.CPU.cost[e.upgradeTracker.cpu + 1] ~= nil then
					if ply:canAfford(e.upgrades.CPU.cost[e.upgradeTracker.cpu + 1]) then
						...
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Вы улучшили (CPU), введите info, чтобы увидеть новые спецификации!")
						net.Send(ply)
					else
						net.Start("BM2.Client.TerminalPrint")
						net.WriteString("Вы не можете позволить себе это!")
