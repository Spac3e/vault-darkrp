	...
	self.customInstructions = {
		...
		info = {
			...
			action = function(arg1, arg2, instructionTable, ent, consoleDisplay)
				...
				consoleDisplay.history = consoleDisplay.history.."КОЛИЧЕСТВО СЕРВЕРОВ                            "..serverCount.."\n"
				local firstPart = "ЧАСТОТА                              "
				local secondPart = tostring(__round(ent:GetClockSpeed(), 3)).."Ghz"
				consoleDisplay.history = consoleDisplay.history..string.sub(firstPart, 1, string.len(firstPart) - string.len(secondPart))..secondPart.."\n"
				local firstPart = "НАЧИНКА                                    "
				local secondPart = ent:GetCoreCount()
				consoleDisplay.history = consoleDisplay.history..string.sub(firstPart, 1, string.len(firstPart) - string.len(secondPart))..secondPart.."\n"
				consoleDisplay.history = consoleDisplay.history.."Потребляемая мощность          100-800W (MAX)\n"
				consoleDisplay.history = consoleDisplay.history.."Имя модели                  Bitminer Rack\n"
		...
		eject = {
			command = "EJECT",
			description = "Извлеките сервер из корпуса",
			action = function(arg1, arg2, instructionTable, ent, consoleDisplay)
				if arg1 == nil then
					consoleDisplay.history = consoleDisplay.history.."Чтобы вытащить сервер из корпуса, введите 'eject 4'\n"
				elseif isnumber(tonumber(arg1)) then
					...
					consoleDisplay.history = consoleDisplay.history.."Данной команды нету '"..arg1.."' допустимые команды 'eject 1-8'\n"
				end
			end
		},
		servers = { 
			command = "SERVERS",
			description = "Выводит список серверов в корпусе.",
			action = function(arg1, arg2, instructionTable, ent, consoleDisplay)
