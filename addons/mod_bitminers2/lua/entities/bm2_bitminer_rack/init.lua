	...
	if state then
		if not self:GetIsMining() then
			if self:GetHasPower() and self:HasServer() then
				...
				return "Вы запустили добычу Bitcoin'ов."
			elseif not self:HasServer() then

				return "В корпусе нету подключенных серверов. Пожалуйста, добавьте хотя бы один, чтобы начать добычу."
			else
				return "Не удается начать добычу Bitcoin'а. Питание не получено."
			end
		else
			return "добыча Bitcoin'а уже запущена."
		end
	else
		if self:GetIsMining() then
			self.miningState = false
			return "Сервер прекратил добычу."
		else
			return "Сервер уже перестал добывать Bitcoin."
		end
	end
end
