...
function ENT:Initialize()
	self:SetModel('models/sup/printerrack/sup_printerrack_large.mdl')
	...
	self.rack = {}
	self.rackStart = 5.5 -- высота первого маника
	self.rackOffset = 19.7 -- высота следующего маника
...
function ENT:StartTouch(e)
	if e:GetClass() == "money_printer" then
		...
		if hasSpace ~= -1 then
			...
			pos = pos - self:GetAngles():Forward() * 2.56
			pos = pos + self:GetAngles():Right() * 21.5
        ...
        if hasSpace > 4 then
            local pos = self:GetAngles():Up() * -13.33 * self.rackStart
			pos = pos + self:GetAngles():Up() * (self.rackOffset * (hasSpace - 1))
			pos = pos - self:GetAngles():Forward() * 2.56
