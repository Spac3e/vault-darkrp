...
local function GetEntityInfo(ent)
	...
	elseif (c == "spawned_food") then
		...
		title = "Еда"
		subtitle = "Энергия: " .. tab.FoodEnergy
	elseif (c == "foods") then
		tab.FoodEnergy = ent.FoodEnergy
		title = "Еда"
		subtitle = "Энергия: " .. tab.FoodEnergy
...
rp.AddCommand('invdrop', function(p, a, s)
...
function SWEP:PrimaryAttack()
	...
	if (self.Owner:EyePos():Distance(tr.HitPos) > 80) then return end
	local ph = ent:GetPhysicsObject()
	if (!ph:IsValid()) then return end

	if (!wl[ent:GetClass()]) then
		self.Owner:ChatPrint("Вы не можете положить " .. ent:GetClass() .. " в свой инвентарь.")
		return
	end
	...
	if (table.Count(p) >= Limit) then
		self.Owner:ChatPrint("Ваш инвентарь полон.")
		return
	end
...
net.Receive("Pocket.AdminDelete", function(len, pl)
	...
	local item = inv[id]
	local itemName = (item.contents and rp.shipments[item.contents].name .. ' (Кол-во: ' .. item.count .. ')') or rp.inv.Wl[item.Class]
