...
net.Receive("Pocket.Load", function()
	...
	for k, v in pairs(contents) do
		if v.contents and (v.contents ~= 0) then
			rp.inv.Add(k, rp.shipments[v.contents].name, 'Кол-во: ' .. v.count, v.Model);
		elseif v.contents and (v.contents == 0) then
			rp.inv.Add(k, 'Пустая коробка', 'Кол-во: 0', v.Model)
		else
...
net.Receive("Pocket.Inspect", function()
	...
	for k, v in pairs(contents) do
		...
		if (v.contents and v.contents != 0) then
			...
			v.SubTitle = 'Кол-во: ' .. v.count
		elseif (v.contents and v.contents == 0) then
			v.Title = 'Пустая коробка'
			v.SubTitle = 'Кол-во: 0'
		else
