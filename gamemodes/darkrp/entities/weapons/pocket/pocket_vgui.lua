...
function PANEL:Finalize()
	...
	if (self.Inspecting) then
		...
		self.InspLbl = ui.Create("DLabel", function(lbl)
			lbl:SetText("Осмотр Инвентаря (" .. self.Inspecting .. ")")
			lbl:SizeToContents()
...
function PANEL:Paint(w, h)
  ...
  draw.SimpleText(table.Count(self.Contents) .. /*'/' .. (LocalPlayer():GetUpgradeCount('pocket_space_2') * 2) + 8 ..*/ ' Предметов', 'ui.22', ScrW()/2, (self.YPos or -20) - 5, rp.col.White, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
end
...
function ITEM:OnMousePressed(mb)
	if (!self:GetParent().Inspecting and mb == MOUSE_LEFT) then
		rp.inv.DisableMenu()
		cmd.Run('invdrop', tostring(self.ID))
