...
SWEP.WorldModel             = "models/custom/ballisticshield.mdl"
SWEP.Secondary.Automatic    = false
SWEP.Secondary.Ammo         = 'none'
SWEP.HoldType               = 'normal'
SWEP.Tooltips	            = {LeftClick = 'Толкнуть', RightClick = 'Толкнуть'}
...
function SWEP:PrimaryAttack()
    if SERVER then
        ...
        if self.Owner:InSpawn() then
            rp.Notify(self.Owner, NOTIFY_ERROR, "Щит не доступен на спавне")
...
if SERVER then
    hook.Add("PlayerSwitchWeapon", "Dontallowriotshields", function(ply, wep1, wep2)
        if wep2:GetClass() == "weapon_shield" and ply:InSpawn() then
            ...
            rp.Notify(ply, NOTIFY_ERROR, "Щит не доступен на спавне")
