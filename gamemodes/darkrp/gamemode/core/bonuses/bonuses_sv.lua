...
util.AddNetworkString('rp::AbilityUse')
net.Receive('rp::AbilityUse', function(len, ply)
	local ability = net.ReadInt(6)
	if !(ability && rp.abilities.Get(ability)) then return end
	
	ability = rp.abilities.Get(ability)
	
	if ability:InCooldown(ply) then ply:Notify(NOTIFY_ERROR, term.Get('AbilityCooldown')) return end
	if !ability:CanUse(ply) then ply:Notify(NOTIFY_ERROR, term.Get('AbilityCantUse'), ability:CantUseReason(ply)) return end
	if ability:GetPlayTime(ply) > ply:GetPlayTime() then ply:Notify(NOTIFY_ERROR, term.Get('AbilityNotEnoughHours')) return end
	if ability:IsVIP() && !ply:IsVIP() then ply:Notify(NOTIFY_ERROR, term.Get('AbilityVIP')) return end
