...
local function scoreboard_DrawJobArea(self)
	...
	local x = self.lblTeam[1] || 1
	local y = self.lblTeam[2] || 1
	surface.SetTextColor(color_white)
...
local function scoreboard_DrawTimeArea(self)
	...
	local x = self.lblHours[1] || 1
	local y = self.lblHours[2] || 1
	surface.SetTextPos(x - w / 2, y)
	surface.DrawText(t)
end
...
local function scoreboard_DrawPingArea(self)
	...
	local x = self.lblPing[1] || 1
	local y = self.lblPing[2] || 1
	surface.SetTextPos(x - w / 2, y)
	surface.DrawText(t)
end
...
function PANEL:UpdatePlayerData()
	...
	local badgeimg = (ply:IsRoot() and 'icon16/tux.png') or (ply:GetRank() == 'globaladmin' and 'icon16/world.png') or (ply:GetRank() == 'superadmin' and 'icon16/shield_add.png') or (ply:IsAdmin() and 'icon16/shield.png') or (ply:GetRank() == 'superior' and 'hud/logos/logo.png') or (ply:GetRank() == 'superadmin' or ply:GetRank() == 'headadmin' and 'icon16/shield_add.png') or (ply:IsVIP() and ((ply:GetPlayTime() > 800 * 3600 and 'icon16/award_star_gold_1.png') or (ply:GetPlayTime() > 500 * 3600 and 'icon16/award_star_silver_3.png') or (ply:GetPlayTime() > 300 * 3600 and 'icon16/award_star_bronze_2.png') or 'icon16/star.png')) or 'icon16/user.png'
	...
	self.NameText = ((self.Player:GetNickName() and self.Player:GetNickName() ~= '') or name)
